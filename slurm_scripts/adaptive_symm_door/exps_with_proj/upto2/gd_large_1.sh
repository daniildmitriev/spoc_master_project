...
declare -a sample_complexity=(0.1 0.5 1.0 2.0)
...
for ((q=0;q<${#loss[@]};++q)); do
srun python -u /home/dmitriev/spoc_master_project/main.py --root_dir symm_door_new --start_seed ${manual_seed[s]} \
 ...
 --project_on_sphere True --gradient_threshold 5e-10 --test_threshold 2e-2 --n_runs 10 --persistence_time ${tau[t]} \
 --use_nesterov False --loss ${loss[q]} --activation ${activation[c]} --symmetric_door_channel_K ${K[k]}
done
done
done
done
