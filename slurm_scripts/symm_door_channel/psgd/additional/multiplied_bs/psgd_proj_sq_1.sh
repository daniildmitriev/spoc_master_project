...
declare -a sample_complexity=(3.0 3.5 4.0)
...
for ((q=0;q<${#loss[@]};++q)); do
srun python -u /home/dmitriev/spoc_master_project/main.py --root_dir symmetric_door_adaptive --start_seed ${manual_seed[s]} \
 --psgd_adaptive_bs True --batch_size ${batch_size[b]} --sample_complexity ${sample_complexity[a]} --n_test 100 \
 --project_on_sphere True --n_features ${n_features[f]} --n_hidden ${n_hidden[h]} --momentum_factor ${momentum[m]} \
 --verbose_freq 100 --optimizer ${optimizer[o]} --n_epochs ${n_epochs[l]} --lr ${lr[l]} --train_threshold 1e-8 \
 --early_stopping_epochs -1 --gradient_threshold 5e-10 --test_threshold 1e-3 --n_runs 10 --persistence_time ${tau[t]} \
 --data_type symmetric-door --loss ${loss[q]} --activation ${activation[c]} --symmetric_door_channel_K ${K[k]}
done
done
done
done
